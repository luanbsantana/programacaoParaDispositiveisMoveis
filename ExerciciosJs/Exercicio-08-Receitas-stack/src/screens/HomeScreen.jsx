...
import { Text, List, Card, Avatar } from "react-native-paper";
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Card style={{ margin: 10, padding: 5  }}>
              <List.Item
                title={item.nome}
                description={`Preparo: ${item.tempoPreparo} | Porções: ${item.porcoes}`}
                left={() => <Avatar.Image size={24} source={{uri: item.imagem}} />}
